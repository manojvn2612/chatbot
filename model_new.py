...
if __name__ == '__main__':
    ...
    input_data = [[word_to_idx[token] for token in tokens] for tokens in tokenized_data]
    input_data = pad_sequences(input_data, pad_value=0)  # Padding sequences to the same length
    ...
    batch_size = 2
    learning_rate = 0.001
    model = SimpleModel(vocab_size, embedding_dim, max_seq_len)
    ...
    predicted_tokens = [[idx_to_word[idx.item()] for idx in seq] for seq in predicted_indices]
    original_tokens = [[idx_to_word[idx.item()] for idx in seq] for seq in test_data]
